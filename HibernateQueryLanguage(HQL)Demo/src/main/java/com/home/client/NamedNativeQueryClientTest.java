...
public class NamedNativeQueryClientTest {
	...
	private static void GetEmployeeById(Session session) {
		Query<Employee> query = session.createNamedQuery("Employee.byId").setInteger("empId", 2);
	...
	private static void getDatafromDepartment(Session session) {
		Query<Object[]> query = session.createNamedQuery("getTotalSalryOfEmpByDept");
